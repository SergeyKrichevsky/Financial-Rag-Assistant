...
> ⚠️ **To continue this project, also read all documents under:** `docs/HANDOFF/`  
> These include architecture, config references, run guides, and logging specs.
...
## ✅ Phase 4: LLM Integration — **Completed (2025-08-14)**

**Scope:** connect retriever with generation; build local-only RAG pipeline; log runs; test E2E.

**Key accomplishments:**
- [x] Built `llm_integration/` module with:
  - `llm_router.py`: backend router (local stub or OpenAI).
  - `answer_generator.py`: loads system prompt, applies caps, calls LLM, formats outputs.
  - `retriever_bridge.py`: takes retriever output, applies filters, builds prompt-ready context.
  - `config_loader.py`: loads model/retriever/logging config from JSON.
  - `run_logger.py`: saves structured logs to `artifacts/v4/runs/`.
- [x] Implemented CLI & test entrypoints:
  - `cli_ask.py`: CLI interface for questions (`--q`, `--k`, `--show-dev`).
  - `test_generate.py`: E2E test (retriever → generator → logger).
  - `smoke_test_openai.py`: exists but not executed (deferred to Phase 5).
- [x] Configs stored and respected:
  - `model.config.json` (currently uses `"local_stub"`).
  - `configs/rag_config.json` with full structure: `retriever`, `generator`, `logging`.
  - `configs/system_prompt.txt` fully externalized and editable.
- [x] Logging system:
  - `last_run_phase4.json` and optional `runs_history.jsonl` saved under `artifacts/v4/runs/`.
  - Fields include timestamp, model, question preview, context size, source refs, answer length.
- [x] Config-driven behavior:
  - Retrieval count (`k`), per-chapter limits, off-switch flags, caps, tokenizer, logging all handled via JSON.
- [x] Error handling:
  - Friendly fallbacks, prompt-not-found, and tokenizer degradation paths handled.
- [x] File structure audited and documented in `docs/REPO_STRUCTURE_v4.md`.

**Deferred to Phase 5:**
- Switching backend to `"openai"` and running live API calls.
- UI development to toggle Final vs Developer outputs.
- Frontend delivery pipeline (FastAPI, Streamlit, or similar).

---

## 📅 Phase 5: User Interface - Next Steps (Planned)
